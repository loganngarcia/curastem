...
/**
 * Ingestion sources we skip Exa company enrichment for when **all** of a company’s
 * jobs are from these known aggregate/noisy sources.
 *
 * RSS is usually excluded only for specific source rows, not all RSS-type sources.
 * Add rows here for exact `source_type` or source handles as needed.
 */
export const EXA_IGNORED_SOURCING_SOURCE_TYPES: readonly string[] = [
  "edjoin",
  "schoolspring",
  "k12jobspot",
  "higheredjobs",
...
export const EXA_IGNORED_SOURCING_SOURCE_HANDLES: readonly string[] = [
  "insidehighered",
  "crimsoneducation",
];

const EXA_IGNORED_SOURCING_SOURCE_TYPES_IN = EXA_IGNORED_SOURCING_SOURCE_TYPES.map(
  (t) => `'${t.replace(/'/g, "''")}'`
).join(", ");
const EXA_IGNORED_SOURCING_SOURCE_HANDLES_IN = EXA_IGNORED_SOURCING_SOURCE_HANDLES.map(
  (h) => `'${h.replace(/'/g, "''")}'`
).join(", ");
const EXA_IGNORED_SOURCE_TYPE_CLAUSE =
  EXA_IGNORED_SOURCING_SOURCE_TYPES.length > 0
    ? `s_ej.source_type IN (${EXA_IGNORED_SOURCING_SOURCE_TYPES_IN})`
    : "0";
const EXA_IGNORED_SOURCE_HANDLE_CLAUSE =
  EXA_IGNORED_SOURCING_SOURCE_HANDLES.length > 0
    ? `(s_ej.source_type = 'rss' AND s_ej.company_handle IN (${EXA_IGNORED_SOURCING_SOURCE_HANDLES_IN}))`
    : "0";
const EXA_IGNORED_SOURCE_CLAUSE = `(${EXA_IGNORED_SOURCE_TYPE_CLAUSE} OR ${EXA_IGNORED_SOURCE_HANDLE_CLAUSE})`;

const EXA_IGNORED_SOURCING_EXCLUSION_SQL = `AND NOT (
  (SELECT COUNT(*) FROM jobs j_all WHERE j_all.company_id = c.id) >= 1
  AND (SELECT COUNT(*) FROM jobs j_ej INNER JOIN sources s_ej ON s_ej.id = j_ej.source_id
       WHERE j_ej.company_id = c.id AND ${EXA_IGNORED_SOURCE_CLAUSE})
   = (SELECT COUNT(*) FROM jobs j_all2 WHERE j_all2.company_id = c.id)
)`;
    ...
    .prepare(
      `SELECT c.* FROM companies c
       ...
       WHERE c.exa_social_enriched_at IS NULL
         AND j.job_count >= 1
         ${EXA_IGNORED_SOURCING_EXCLUSION_SQL}
...
/**
 * True when all of a company’s jobs use only {@link EXA_IGNORED_SOURCING_SOURCE_TYPES}. Used to
 * skip Exa on the enrichment queue without a schema flag.
 */
export async function companyHasOnlyExaIgnoredSourcingJobs(
  db: D1Database,
  companyId: string
): Promise<boolean> {
  const row = await db
    .prepare(
      `SELECT
         (SELECT COUNT(*) FROM jobs WHERE company_id = ?) AS total,
         (SELECT COUNT(*) FROM jobs j INNER JOIN sources s ON s.id = j.source_id
         WHERE j.company_id = ?
           AND (
             s.source_type IN (${EXA_IGNORED_SOURCING_SOURCE_TYPES_IN})
             OR (s.source_type = 'rss' AND s.company_handle IN (${EXA_IGNORED_SOURCING_SOURCE_HANDLES_IN}))
           )) AS ignored_sourcing_count`
    )
    .bind(companyId, companyId)
    .first<{ total: number; ignored_sourcing_count: number }>();
  if (!row || row.total < 1) return false;
  return row.ignored_sourcing_count === row.total;
}
...
/**
 * Ingestion source types for K-12 and higher-ed *job boards* (used to scope Wikidata website backfill).
 * Excludes generic ATS host types (e.g. jobsyn, hirebridge): those are not education-specific.
 * RSS: only the Inside Higher Ed feed — other RSS boards are excluded.
 */
export const WIKIDATA_EDUCATION_SOURCE_TYPES: readonly string[] = [
  "edjoin",
  "schoolspring",
  "k12jobspot",
  "higheredjobs",
...
export const WIKIDATA_EDUCATION_RSS_SOURCE_HANDLES: readonly string[] = ["insidehighered"];

const WIKI_EDU_SRC_PLACEHOLDERS = WIKIDATA_EDUCATION_SOURCE_TYPES.map(() => "?").join(", ");
const WIKI_EDU_RSS_PLACEHOLDERS = WIKIDATA_EDUCATION_RSS_SOURCE_HANDLES.map(() => "?").join(", ");

/**
 * Companies with jobs from education sources, no `website_url`, eligible for a free Wikidata (P856) pass.
 * Retries throttled by `wikidata_website_attempted_at` and `retryIfAttemptedBeforeEpoch`.
 */
export async function listCompaniesForWikidataWebsiteFill(
  db: D1Database,
  ...
  const result = await db
    .prepare(
      `SELECT c.* FROM companies c
       WHERE (c.website_url IS NULL OR TRIM(c.website_url) = '')
         AND IFNULL(c.website_infer_suppressed, 0) = 0
         AND (c.wikidata_website_attempted_at IS NULL OR c.wikidata_website_attempted_at < ?)
         AND EXISTS (
           SELECT 1 FROM jobs j
           INNER JOIN sources s ON s.id = j.source_id
           WHERE j.company_id = c.id
             AND (
               s.source_type IN (${WIKI_EDU_SRC_PLACEHOLDERS})
               OR (s.source_type = 'rss' AND s.company_handle IN (${WIKI_EDU_RSS_PLACEHOLDERS}))
             )
         )
       ORDER BY c.wikidata_website_attempted_at ASC NULLS FIRST, c.updated_at ASC
       LIMIT ?`
    )
    .bind(
      retryIfAttemptedBeforeEpoch,
      ...WIKIDATA_EDUCATION_SOURCE_TYPES,
      ...WIKIDATA_EDUCATION_RSS_SOURCE_HANDLES,
      limit
    )
    .all<CompanyRow>();
  return result.results ?? [];
...
/** After Wikidata website lookup: set URL + probe time on success, or only attempted-at on miss. */
export async function updateCompanyWikidataWebsiteResult(
  db: D1Database,
  id: string,
  patch: {
    wikidata_website_attempted_at: number;
    website_url?: string;
    /**
     * When `website_url` is set: omit to default `website_checked_at` to `wikidata_website_attempted_at`;
     * pass `null` to leave unchecked so the website HTTP probe batch can verify later.
     */
    website_checked_at?: number | null;
  }
): Promise<void> {
  const now = Math.floor(Date.now() / 1000);
  if (patch.website_url !== undefined) {
    const checkedAt: number | null =
      patch.website_checked_at === undefined
        ? patch.wikidata_website_attempted_at
        : patch.website_checked_at;
    await db
      .prepare(
        `UPDATE companies SET
           website_url = ?,
           website_checked_at = ?,
           wikidata_website_attempted_at = ?,
           updated_at = ?
         WHERE id = ?`
      )
      .bind(patch.website_url, checkedAt, patch.wikidata_website_attempted_at, now, id)
      .run();
    return;
  }
  await db
    .prepare(
      `UPDATE companies SET wikidata_website_attempted_at = ?, updated_at = ? WHERE id = ?`
    )
    .bind(patch.wikidata_website_attempted_at, now, id)
    .run();
}
...
// ─────────────────────────────────────────────────────────────────────────────
// User data tables (idempotent cold-boot guard)
// ─────────────────────────────────────────────────────────────────────────────
// Called from the auth/sync routes on first hit so we don't require anyone to
// re-run schema.sql after a deploy. Adds any columns that a pre-v2 `chats`
// table might be missing, creates `chat_messages` + `tombstones`, and does a
// one-time backfill from any rows that still have `payload_json`.
//
// Safe characteristics:
//   - All statements are CREATE … IF NOT EXISTS or ALTER TABLE ADD COLUMN
//     guarded by a catch (SQLite errors out on duplicate columns; we swallow).
//   - Backfill reads rows that have `payload_json` AND no messages yet, so
//     re-runs are no-ops.
//   - Never drops columns — the old `payload_json` / `content_hash` remain on
//     any pre-v2 row. They are still populated as compatibility aliases because
//     some deployed D1 tables have them NOT NULL.

let _userDataTablesReadyPromise: Promise<void> | null = null;
let _lastReadyDb: D1Database | null = null;

/** Idempotent. Call on any auth/sync entry point; caches within a single isolate. */
export async function ensureUserDataTables(db: D1Database): Promise<void> {
  if (_lastReadyDb === db && _userDataTablesReadyPromise) {
    return _userDataTablesReadyPromise;
  }
  _lastReadyDb = db;
  _userDataTablesReadyPromise = (async () => {
    // Core tables (no-ops if schema.sql has already created them).
    await db
      .prepare(
    ...
    await db
      .prepare(
        `CREATE TABLE IF NOT EXISTS profile (
          ...
          you_work                  TEXT,
          you_interests             TEXT,
          dismissed_interest_chips  TEXT,
          resume_plain              TEXT,
          resume_doc_html           TEXT,
          resume_file_r2_key        TEXT,
    ...
    // Chats — base shape. For pre-v2 databases the CREATE is a no-op; we then
    // additively add the v2 columns below.
    await db
      .prepare(
    ...
    // Add v2 columns to any pre-v2 chats table (silently skip on duplicates).
    for (const sql of [
      `ALTER TABLE chats ADD COLUMN last_message_at INTEGER`,
      ...
      `ALTER TABLE chats ADD COLUMN next_seq INTEGER NOT NULL DEFAULT 1`,
      `ALTER TABLE chats ADD COLUMN meta_json TEXT NOT NULL DEFAULT '{}'`,
      `ALTER TABLE chats ADD COLUMN meta_hash TEXT NOT NULL DEFAULT ''`,
      `ALTER TABLE chats ADD COLUMN payload_json TEXT NOT NULL DEFAULT '{}'`,
      `ALTER TABLE chats ADD COLUMN content_hash TEXT NOT NULL DEFAULT ''`,
    ]) {
      try {
        await db.prepare(sql).run();
      } catch {
    ...
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_chats_user_pinned_updated
          ON chats (user_id, is_pinned DESC, pinned_at DESC, updated_at DESC, id)`
      )
      .run();
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_chats_user_hash
          ON chats (user_id, meta_hash)`
      )
      .run();
    ...
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_docs_user_updated
          ON docs (user_id, updated_at DESC, id)`
      )
      .run();
    ...
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_apps_user_updated
          ON apps (user_id, updated_at DESC, id)`
      )
      .run();
    ...
    await db
      .prepare(
        `CREATE INDEX IF NOT EXISTS idx_tombstones_user_deleted
          ON tombstones (user_id, deleted_at DESC)`
      )
      .run();
    ...
    // Additive column guards for pre-existing users tables.
    for (const ddl of [
      `ALTER TABLE users ADD COLUMN scheduled_delete_at INTEGER`,
      `ALTER TABLE users ADD COLUMN email_job_alerts INTEGER NOT NULL DEFAULT 1`,
      `ALTER TABLE users ADD COLUMN email_local_events INTEGER NOT NULL DEFAULT 1`,
      `ALTER TABLE users ADD COLUMN email_announcements INTEGER NOT NULL DEFAULT 1`,
    ]) {
      try {
    ...
    // One-shot backfill: for any pre-v2 chat rows that still have a
    // `payload_json` column but no rows in chat_messages yet, split the
    // messages array out. A pre-v2 schema may not have `payload_json` at all
    // (fresh v2 deploys); the catch handles that.
    try {
      await backfillChatMessagesFromPayloadJson(db);
    } catch {
      /* table has no payload_json column — nothing to backfill */
    }
  })();
  return _userDataTablesReadyPromise;
}
...
/**
 * Sweep users whose scheduled_delete_at is in the past and hard-delete them.
 * Called from the hourly cron. Best-effort R2 cleanup runs first.
...
export async function sweepScheduledDeletions(
  ...
  const due = await db
    .prepare(
      `SELECT u.id, p.resume_file_r2_key
         FROM users u
         LEFT JOIN profile p ON p.user_id = u.id
        WHERE u.scheduled_delete_at IS NOT NULL
          AND u.scheduled_delete_at <= ?`
    )
    .bind(nowSec)
    .all<{ id: string; resume_file_r2_key: string | null }>();

  let deleted = 0;
  for (const row of due.results ?? []) {
    if (row.resume_file_r2_key) {
      try {
        await r2.delete(row.resume_file_r2_key);
      } catch { /* best-effort */ }
    }
...
async function backfillChatMessagesFromPayloadJson(db: D1Database): Promise<void> {
  // Probe: does chats still have payload_json?
  const info = await db.prepare(`PRAGMA table_info(chats)`).all<{ name: string }>();
  const hasPayload = (info.results ?? []).some((r) => r.name === "payload_json");
  if (!hasPayload) return;

  // Only touch chats that still have their messages embedded and are NOT yet
  // represented in chat_messages. Cap per run so a huge corpus doesn't blow
  // the worker CPU budget; subsequent calls continue where we left off.
  const PAGE = 50;
  const { results } = await db
    .prepare(
      `SELECT c.id, c.user_id, c.created_at, c.updated_at, c.payload_json
         FROM chats c
  ...
  for (const row of results ?? []) {
    ...
    let lastMs = row.updated_at * 1000;
    for (const m of msgs) {
      if (!m || typeof m !== "object") continue;
      const role = typeof m.role === "string" ? m.role : "assistant";
      const contentJson = JSON.stringify(m);
      const hash = await sha256Hex(canonicalizeJson(m));
      // Reconstruct a plausible millis timestamp: prefer m.timestamp, else
      // spread messages across the chat lifetime deterministically.
      const ts =
        typeof m.timestamp === "number"
          ? m.timestamp
          : createdAtMs + seq; // +1ms per message to preserve order
      lastMs = Math.max(lastMs, ts);
      lastText = extractPreview(m);
      stmts.push(
        db
          .prepare(
          ...
          .bind(row.id, row.user_id, seq, ts, role, contentJson, hash)
      );
      seq++;
    }

    // Build meta_json from the non-message fields, write metadata back.
    const meta = {
      suggestions: Array.isArray(payload.suggestions) ? payload.suggestions : [],
      notes: typeof payload.notes === "string" ? payload.notes : "",
      docType: payload.docType ?? null,
      docCompany: payload.docCompany ?? null,
      activeDocId: payload.activeDocId ?? null,
      refs: payload.refs ?? { docs: [], app: null, whiteboard: null },
    };
    const metaJson = JSON.stringify(meta);
    const metaHash = await sha256Hex(canonicalizeJson(meta));

    stmts.push(
      db
        .prepare(
          `UPDATE chats
              SET message_count = ?, next_seq = ?, last_message_at = ?, last_message_preview = ?,
                  meta_json = ?, meta_hash = ?
            WHERE id = ?`
        )
        .bind(
    ...
    // D1 batch cap is 100. Our worst case is ~msgs.length + 1 per chat.
...
function extractPreview(m: { content?: unknown }): string | null {
  const c = m?.content;
...
/**
 * Companies that need HQ coordinates: **no lat yet**, have `hq_city` or `hq_address`,
 * and have not failed geocoding. Cron tries **Mapbox** (good `feature_type` only) then
 * **Google Places**; see `placesGeocodeCompanies.ts`.
 *
 * The hq_geocode_failed_at guard prevents retrying companies whose location
 * data genuinely doesn't match anything. It is cleared by
...
export async function listCompaniesNeedingPlacesGeocode(
  ...
  const { results } = await db
    .prepare(
      `SELECT c.id, c.name, c.hq_city, c.hq_country, c.hq_address
       FROM companies c
       WHERE c.hq_lat IS NULL
         AND c.hq_geocode_failed_at IS NULL
         AND (c.hq_city IS NOT NULL OR c.hq_address IS NOT NULL)
