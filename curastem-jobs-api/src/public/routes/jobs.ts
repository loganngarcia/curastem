...
} from "../../shared/utils/jobSearchQuery.ts";
import { buildPublicSalary, embedQuery } from "../../shared/enrichment/ai.ts";
import type { Env, PublicJob } from "../../shared/types.ts";
...
} from "../../shared/utils/jobsListCache.ts";
  ...
  if (q && !titleForSearch && !company && env.JOBS_VECTORS && env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
    try {
      ...
      const getEmbedding = async (title: string): Promise<number[]> => {
        ...
        const quota = await reserveGeminiQuota(env.RATE_LIMIT_KV, "public_jobs_embedding");
        if (!quota.allowed) {
