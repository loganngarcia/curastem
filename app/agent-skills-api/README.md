...
**Primary consumer**: The [Curastem](https://curastem.org) mentorship component (`app/web.tsx`), a Framer code component that embeds an AI-powered mentorship chat.
...
|------------------|----------|-------------|
| `ALLOWED_ORIGINS`| No       | Extra comma-separated allowed origins. Built-ins already include Curastem, Framer preview domains, `*.framer.ai`, and local dev. Use `*` to allow all for testing only. |
...
From `app/agent-skills-api/`:

1. **Log in**: `npx wrangler login`
2. **Deploy**: `npm run deploy`
3. **Set `ALLOWED_ORIGINS`** (optional extra origins):
   ```bash
   # Allow all (testing only)
   echo "*" | npx wrangler secret put ALLOWED_ORIGINS

   # Extra production domains beyond the built-in Curastem and Framer allowlist
   echo "https://preview.example.com" | npx wrangler secret put ALLOWED_ORIGINS
   ```
...
## Integration with Curastem (`app/web.tsx`)
