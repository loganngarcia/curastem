...
export interface Env {
  ...
  /**
   * R2 bucket storing per-user uploads (currently: resume files).
   * Objects are addressed as `users/{user_id}/resume/{uuid}-{filename}`.
   * The bucket is private; clients never touch R2 directly — they go through
   * presigned URLs minted by app/uploads/handlers.ts so the Worker enforces ownership.
   */
  USER_FILES: R2Bucket;
  /**
   * Firebase project id (e.g. "curastem"). Used by auth/firebaseVerify.ts to
   * validate the `aud` and `iss` claims on incoming Firebase ID tokens.
   * Set via: wrangler secret put FIREBASE_PROJECT_ID
   */
  FIREBASE_PROJECT_ID: string;
  /**
   * HMAC key used to derive the session-token hash stored in the `sessions` table.
   * 32+ random bytes base64-encoded. Rotating this key invalidates every session.
   * Set via: wrangler secret put SESSION_SIGNING_KEY
   */
  SESSION_SIGNING_KEY: string;
  /**
   * Discord / Slack / generic incoming webhook URL for admin notifications.
   * When set, every data-export request (including deferred ones that need
   * manual follow-up) sends a ping. If unset, requests are logged only.
   * Set via: wrangler secret put ADMIN_WEBHOOK_URL
   */
...
export interface CompanyRow {
  ...
  /** When non-zero, Exa may not set `website_url` after a failed/dead-URL website probe. */
  website_infer_suppressed: number;
  /**
   * Last Wikidata (P856) website lookup for education employers. Throttles retries when
   * no verifiable website was stored. See `enrichment/wikidataEducationWebsite.ts`.
   */
  wikidata_website_attempted_at: number | null;
...
// ─────────────────────────────────────────────────────────────────────────────
// User accounts — auth + cross-device sync (see schema.sql "USER ACCOUNTS")
// ─────────────────────────────────────────────────────────────────────────────

/**
 * A signed-in user. `id` is our own UUID — treat it as the only durable handle.
 * `google_sub` is the portable identifier from Google; `firebase_uid` is
 * secondary and provider-specific so we stay portable to Clerk/Auth0/etc.
 */
export interface UserRow {
  ...
  /** Unix seconds when this account is scheduled for hard deletion, or null. */
  scheduled_delete_at: number | null;
  /** Email preference flags — 1 = opted in (default), 0 = opted out. */
  email_job_alerts: number;
  email_local_events: number;
...
export interface ProfileRow {
  ...
  /** Newline-separated bullets, matches localStorage "you_interests" format. */
  you_interests: string | null;
  /** JSON array. */
  dismissed_interest_chips: string | null;
...
/**
 * Chat metadata. Messages are NOT stored here; they live in chat_messages.
 *
 * - `updated_at` (seconds) moves on any sidebar-visible change: new message,
 *   title edit, pin toggle, meta edit. That's what the sidebar sorts by.
 * - `last_message_at` (millis) is denormalized so the sidebar lists N chats
 *   with a single SELECT; `last_message_preview` is the truncated text shown
 *   under the title.
 * - `meta_hash` is a sha256 over the NON-message fields only (title, pin, and
 *   canonical meta_json). Message changes do NOT change meta_hash — that way
 *   streaming doesn't churn the hash and content-hash dedup on sign-in works
 *   even when the last message is still being written.
 */
export interface ChatRow {
  ...
  created_at: number;
  updated_at: number;
  last_message_at: number | null;   // millis
  last_message_preview: string | null;
  message_count: number;
  next_seq: number;                 // next seq to assign on insert
  meta_json: string;                // { suggestions, notes, docType, docCompany, activeDocId, refs }
  meta_hash: string;
}

/**
 * One message in a chat. seq is per-chat monotonic; created_at is MILLIS so
 * two messages written in the same second still sort deterministically.
 * content_json is opaque — matches the client's Message interface in web.tsx.
 */
...
/**
 * Record of a user-initiated delete. Written when the client deletes locally
 * while offline, and honored on the next /sync/delta push so the account copy
 * is removed too (prevents "deleted chat came back" bugs).
 *
 * For kind='message' the entity_id is `${chat_id}:${seq}`.
 */
export interface TombstoneRow {
  user_id: string;
  kind: "chat" | "doc" | "app" | "message";
  entity_id: string;
  deleted_at: number;   // millis
}

/** One row per ChatDocEntry (discriminator on `kind`). */
...
/** Mini-app (ChatSession.app) or whiteboard (ChatSession.whiteboard). */
export interface AppRow {
  ...
  kind: "app" | "whiteboard";
  title: string | null;
  /** For `app`: {code, mode}. For `whiteboard`: the raw whiteboard JSON. */
  payload_json: string;
  content_hash: string;
  created_at: number;
  updated_at: number;
...
/** Backing store for the curastem_session cookie (and Bearer fallback). */
export interface SessionRow {
  /** sha256 hex of the opaque 32-byte token; raw token is NEVER stored. */
  token_hash: string;
  user_id: string;
  created_at: number;
  expires_at: number;
...
export type SourceType =
  ...
  */
  ...
  | "gusto_recruiting"
  /**
   * EDJOIN (K–12) — unauthenticated `GET /Home/LoadJobs?stateID=…&page=…&rows=…` JSON (same
   * contract as the public Jobs UI). `base_url` must include `stateID` (e.g. `?stateID=24` for
   * California). List rows omit full descriptions; the fetcher synthesizes text from title,
   * district, and location. See edjoin.ts.
   */
  | "edjoin"
  /**
   * SchoolSpring (PowerSchool) — public `api.schoolspring.com` list + per-job detail JSON; same
   * contract the `www.schoolspring.com` SPA uses. `base_url` is the board page; optional `?location=…`
   * and other search params are forwarded. See schoolspring.ts.
   */
  | "schoolspring"
  /**
   * K12JobSpot (Frontline) — `POST https://api.k12jobspot.com/api/Jobs/Search` + `GET /api/Jobs/{id}`.
   * `base_url` must include `?state=XX` (US postal code) and may include `?keywords=…` or
   * `?locality=…`. See k12jobspot.ts.
   */
  | "k12jobspot"
  /**
   * HigherEdJobs — HTML `details.cfm?JobCode=` + `JobPosting` JSON-LD. Listings via category RSS
   * or `search.cfm`. Plain `fetch` only. See higheredjobs.ts.
   */
  | "higheredjobs"
  /**
   * Chronicle of Higher Education Jobs — `jobs.chronicle.com` RSS (`/jobsrss/`) + per-job
   * `JobPosting` JSON-LD, or the Jina Reader markdown mirror if Cloudflare blocks direct fetch. See
   * chronicle_jobs.ts.
   */
  | "chronicle_jobs"
  /**
   * Jobsyn ATS (Pearson-style): listings from Jobsyn Solr API + `microsites.dejobs.org` detail JSON.
   */
  | "jobsyn"
  /**
   * Hirebridge career centers (ASP.NET job list + ASPX job detail pages).
   * Listings are `v3/Jobs/list.aspx?cid=...` pages with per-job
   * `JobDetails.aspx?jid=...` detail pages.
   */
  | "hirebridge";
